	...
	public class CecilLabel {
		...
		public void Emit(OpCode opCode) {
			...
			if (PlaceholderSet) {
				...
				var firstInstr = Enumerable.FirstOrDefault<Instruction>(instrs);
				if (firstInstr == null) {
					needsFirstInstrFixup = true;
					_ilp.Emit(OpCodes.Nop);
					firstInstr = Enumerable.First<Instruction>(instrs);
				}
				_ilp.Emit(opCode, firstInstr);
				var emitted = Enumerable.Last<Instruction>(instrs);
				if (needsFirstInstrFixup) {
					emitted.Operand = emitted;
					instrs.RemoveAt(0);
				}
			}
			_emitted.AddLast(Enumerable.Last<Instruction>(_ilp.Body.Instructions));
		}

		public void Mark() {
			_ilp.Emit(OpCodes.Nop);
			_placeholder = Enumerable.Last<Instruction>(_ilp.Body.Instructions);
