<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="logoImage">
		<picture>
			<xsl:if test="normalize-space($logoFileSvg)">
				<source>
					<includeAttribute name="srcset" item="iconPath">
						<parameter>
							<xsl:value-of select="$logoFileSvg"/>
						</parameter>
					</includeAttribute>
				</source>
			</xsl:if>
	...
	<xsl:template name="t_autogenSeeAlsoLinks">
		...
		<xsl:variable name="v_namespaceId">
			<xsl:choose>
				<xsl:when test="normalize-space(/document/reference/containers/namespace/@api)='( Default Namespace )'">
	...
	<xsl:template name="t_putSection">
		...
		<xsl:if test="normalize-space($p_title)">
			<div class="collapsibleAreaRegion">
				...
				<span id="{$toggleImage}" class="collapsibleRegionTitle" onclick="SectionExpandCollapse('{$togglePrefix}')"
	...
	<xsl:template name="t_putAlert">
		...
		<xsl:variable name="v_noteImgName">
			<xsl:choose>
				<xsl:when test="$p_alertClass='note' or $p_alertClass='tip' or $p_alertClass='implement' or $p_alertClass='caller' or $p_alertClass='inherit'">
					<xsl:text>AlertNote</xsl:text>
				</xsl:when>
				<xsl:when test="$p_alertClass='caution' or $p_alertClass='warning'">
					<xsl:text>AlertCaution</xsl:text>
				</xsl:when>
				<xsl:when test="$p_alertClass='security' or $p_alertClass='security note'">
					<xsl:text>AlertSecurity</xsl:text>
				</xsl:when>
				<xsl:when test="$p_alertClass='important'">
					<xsl:text>AlertCaution</xsl:text>
				</xsl:when>
					...
					<th>
						<picture>
							<source>
								<includeAttribute name="srcset" item="iconPath">
									<parameter>
										<xsl:value-of select="concat($v_noteImgName,'.svg')"/>
									</parameter>
								</includeAttribute>
							</source>
							<img>
								<includeAttribute name="src" item="iconPath">
									<parameter>
										<xsl:value-of select="concat($v_noteImgName,'.png')"/>
									</parameter>
								</includeAttribute>
