...
>
	...
	<xsl:template match="/" name="t_docbod">
		<html>
			<head>
				<link rel="shortcut icon">
					<includeAttribute name="href" item="iconPath">
						<parameter>favicon.ico</parameter>
					</includeAttribute>
				</link>
				<link rel="stylesheet" type="text/css">
					<includeAttribute name="href" item="stylePath">
						<parameter>branding.css</parameter>
					</includeAttribute>
				</link>
				<link rel="stylesheet" type="text/css">
					<includeAttribute name="href" item="stylePath">
				...
				<script type="text/javascript">
					<includeAttribute name="src" item="scriptPath">
						<parameter>branding.js</parameter>
					</includeAttribute>
					<xsl:text> </xsl:text>
				</script>
		...
		<meta name="container">
			<xsl:attribute name="content">
				<xsl:choose>
					...
					<!-- use 'defaultNamespace' for members in the default namespace (where namespace/@api == 'N:') -->
					<xsl:when test="normalize-space(/document/reference/containers/namespace/@api)">
						<xsl:text>defaultNamespace</xsl:text>
					</xsl:when>
					<!-- for the default namespace topic, use 'defaultNamespace' -->
	...
	<xsl:template match="elements" mode="namespace" name="t_namespaceElements">
		...
		<!-- TODO: support delegates that look like classes -->
	...
	<xsl:template match="element" mode="namespace" name="t_namespaceElement">
		<xsl:variable name="v_typeVisibility">
			<xsl:choose>
				<xsl:when test="typedata/@visibility='family' or typedata/@visibility='family or assembly' or typedata/@visibility='assembly'">Protected</xsl:when>
				<xsl:when test="typedata/@visibility='private'">Private</xsl:when>
				...
				<td>
					<xsl:call-template name="t_putMemberIcons">
						<xsl:with-param name="p_memberVisibility">
							<xsl:choose>
								<xsl:when test="normalize-space($v_publicMember)!=''">Public</xsl:when>
								<xsl:when test="normalize-space($v_protectedMember)!=''">Protected</xsl:when>
								<xsl:when test="memberdata/@visibility='private'">Private</xsl:when>
								<xsl:otherwise>Public</xsl:otherwise>
							</xsl:choose>
						</xsl:with-param>
						<xsl:with-param name="p_staticMember" select="normalize-space($v_staticMember)"/>					</xsl:call-template>
				</td>
				<td>
					<xsl:choose>
	...
	<xsl:template name="t_putTypeIcon">
		...
		<xsl:variable name="typeSubgroup">
			<xsl:value-of select="concat(translate(substring(apidata/@subgroup,1,1),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),substring(apidata/@subgroup,2))"/>
		</xsl:variable>
		
		<xsl:if test="normalize-space($typeSubgroup)=''">
			<xsl:message>
				<xsl:value-of select="apidata/@name"/>
			</xsl:message>
			<xsl:message terminate="yes">typeSubgroup is empty during t_putTypeIcon</xsl:message>
		</xsl:if>
		
		<picture>
			<source>
				<includeAttribute name="srcset" item="iconPath">
					<parameter>
						<xsl:value-of select="concat($typeSubgroup,$p_typeVisibility,'.svg')"/>
					</parameter>
				</includeAttribute>
			</source>
			<img>
				<includeAttribute name="src" item="iconPath">
					<parameter>
						<xsl:value-of select="concat($typeSubgroup,$p_typeVisibility,'.png')"/>
					</parameter>
				</includeAttribute>
		...
		<xsl:if test=".//example">
			<picture>
				<source>
					<includeAttribute name="srcset" item="iconPath">
						<parameter>Code.svg</parameter>
					</includeAttribute>
				</source>
				<img>
					<includeAttribute name="src" item="iconPath">
						<parameter>Code.png</parameter>
					</includeAttribute>
	...
	<xsl:template name="t_putMemberIcons">
		...
		<xsl:variable name="v_memberSubgroup">
			<xsl:choose>
				<xsl:when test="apidata/@subgroup='method'">
					<xsl:choose>
						<xsl:when test="normalize-space(apidata/@subsubgroup)=''">
							<xsl:value-of select="'Method'"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat(translate(substring(apidata/@subsubgroup,1,1),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),substring(apidata/@subsubgroup,2))"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="normalize-space(apidata/@subgroup)=''">
							<xsl:value-of select="'Method'"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat(translate(substring(apidata/@subgroup,1,1),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),substring(apidata/@subgroup,2))"/>
		...
		<xsl:if test="normalize-space($v_memberSubgroup)=''">
			<xsl:message>
				<xsl:value-of select="apidata/@name"/>
			</xsl:message>
			<xsl:message terminate="yes">v_memberSubgroup is empty during t_putMemberIcons</xsl:message>
		</xsl:if>

		<!-- test for explicit interface implementations, which get the interface icon -->
		<xsl:if test="memberdata/@visibility='private' and proceduredata/@virtual='true'">
			<picture class="explicit-implementation">
				<source>
					<includeAttribute name="srcset" item="iconPath">
						<parameter>Interface.svg</parameter>
					</includeAttribute>
				</source>
				<img>
					<includeAttribute name="src" item="iconPath">
						<parameter>Interface.png</parameter>
					</includeAttribute>
		...
		<picture class="member-icon">
			<source>
				<includeAttribute name="srcset" item="iconPath">
					<parameter>
						<xsl:value-of select="concat($v_memberSubgroup,$p_memberVisibility,'.svg')"/>
					</parameter>
				</includeAttribute>
			</source>
			<img>
				<includeAttribute name="src" item="iconPath">
					<parameter>
						<xsl:value-of select="concat($v_memberSubgroup,$p_memberVisibility,'.png')"/>
					</parameter>
				</includeAttribute>
		...
		<xsl:if test="$p_staticMember!=''">
			<picture class="modifier-static">
				<source>
					<includeAttribute name="srcset" item="iconPath">
						<parameter>Static.svg</parameter>
					</includeAttribute>
				</source>
				<img>
					<includeAttribute name="src" item="iconPath">
						<parameter>Static.png</parameter>
					</includeAttribute>
	...
	<xsl:template name="t_putRequirementsInfo">
		...
		<xsl:text>&#xa0;</xsl:text>
		<!-- TODO: handle default namespace renaming -->
	...
	<xsl:template match="platforms[platform]"
		...
		<xsl:call-template name="t_putSectionInclude">
			...
			<xsl:with-param name="p_content">
				...
				<xsl:if test="/document/reference/versions/versions[@name='netfw']//version">
					<p>
	...
	<xsl:template name="t_processVersions">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="v_supportedCount" select="count(version[not(@obsolete)] | versions[version[not(@obsolete)]])"/>
