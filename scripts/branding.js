...
else if (curLoc.indexOf("ms-its:") === 0)
	isHelp1 = true;
else
	isHelp1 = false;
...
function UpdateLST(language) {
	for (var lstMember in allLSTSetIds) {
		...
		if (devLangSpan !== null) {
			...
			var devLangSpanPrevSib = devLangSpan.previousSibling;
			if (devLangSpanPrevSib !== null && devLangSpanPrevSib.nodeValue !== null)
				devLangSpanPrevSib.nodeValue = devLangSpanPrevSib.nodeValue.replace(/\s+$/, "");
...
function SetCookie(name, value) {
	...
	var expiresDate = new Date(today.getTime() + (60 * 1000 * 60 * 60 * 24));
...
function SetActiveTab(tabSetId, tabIndex, tabCount) {
	...
	while (i <= tabCount) {
		...
		if (tabTemp !== null) {
			...
			else if (tabTemp.className === "codeSnippetContainerTabPhantom")
...
function CopyToClipboard(tabSetId) {
	...
	if (window.clipboardData) {
		...
		try {
			...
			alert(
				"Permission denied. Enter \"about:config\" in the address bar and double-click the \"signed.applets.codebase_principal_support\" setting to enable copying to the clipboard.");
		}
	}
}
...
function SectionExpandCollapse(togglePrefix) {
	...
	if (image !== null && section !== null)
		if (section.style.display === "") {
			image.className = image.className.replace("Expanded", "Collapsed");
			section.style.display = "none";
		}
		else {
			image.className = image.className.replace("Collapsed", "Expanded");
